...
בפייתון, כל קובץ הוא גם **מוד**ול וגם **תוכנית בפני עצ**מה. כשאתה מריץ קובץ ישירות, משתנה פנימי בשם __name__ מקבל את הערך "__main__". אבל כשקובץ מיובא כמודול ממקום אחר, הוא יקבל את שמו האמיתי, לדוגמה "text_utils". וזה בדיוק מה שמאפשר להפריד בין **קוד להרצ**הלבין **קוד לשימוש חוזר.**
**דוגמה פשוטה**
```python
# text_utils.py
def normalize(text: str) -> str:
